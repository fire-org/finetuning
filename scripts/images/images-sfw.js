...
const promptTemplate = Handlebars.compile(fs.readFileSync(path.join(__dirname, 'orion-sfw.hbs'), 'utf8'));
...
async function main() {
  ...
  for(let i = 0; i < max;) {
    async function newRound() {
      ...
      data.push({
        type: 'orion-sfw-images',
        tag: 'orion-sfw-images',
        user_gender: 'male',
        bot_name: model['Name'],
        bot_age: model['Age'],
        bot_character: {
    ...
    fs.writeFileSync(
      path.join(__dirname, 'sfw-images.json'),
