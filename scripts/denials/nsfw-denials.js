...
const promptTemplate = Handlebars.compile(fs.readFileSync(path.join(__dirname, 'orion-nsfw-denials.hbs'), 'utf8'));
...
async function complete({model, topic}, denialTopic) {
  const rows = []
...
async function main() {
  ...
  for(let i = 0; i < max;) {
    async function newRound() {
      const { model, topic } = generateEntropy()

      denialTopic = pickRandom([
        'sadomasochism',
        // 'necrophilia',
        // 'bestiality',
        // 'rape',
        // 'children',
        // 'scat play',
        // 'celebrity ' + pickRandom(['Taylor Swift', 'Cardi B', 'Dua Lipa', 'Zendaya', 'Selena Gomez', 'Angelina Jolie', 'Cameron Diaz', 'Jennifer Aniston', 'Emma Watson', 'Shakira', 'Margot Robbie'])
      ])
      // denialTopic = 'scat play'
      console.log('new round with', denialTopic)

      const {systemPrompt, conversation} = await complete({model, topic}, denialTopic)
      
      data.push({
        type: 'orion-nsfw-denials',
        tag: 'orion-sfw-denials',
        denial_topic: denialTopic,
        user_gender: 'male',
        bot_name: model['Name'],
        bot_age: model['Age'],
    ...
    fs.writeFileSync(
      path.join(__dirname, 'nsfw-denials.json'),
